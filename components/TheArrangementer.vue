<template>
    <div id="arrangementer" class="scroll-mt-24">
        <h2 class="sm:text-4xl text-2xl font-bold text-white text-center">
            {{ $t("arrangementer.title") }}
        </h2>
        <div
            class="flex gap-8 items-center justify-start md:justify-center relative my-7 px-8 sm:px-20 overflow-hidden"
        >
            <!-- Venstre pil -->
            <button
                @click="slideLeft"
            ...
            >
                <img
                    src="/icons/pil.svg"
                    :alt="$t('arrangementer.prev')"
                    class="sm:w-10 sm:h-10 w-6 h-6 rotate-180 cursor-pointer transition"
                />
            </button>
            ...
            >
                <div
                    class="hidden md:flex md:w-6/12 max-w-lg max-h-100 justify-center overflow-hidden"
            ...
            >
                <div
                    ref="textBoxRef"
                    class="md:w-6/12 border-neon-neonred p-6 flex flex-col justify-start md:justify-center"
                    :key="slides[current].title"
                    :style="
                        minTextHeight ? { minHeight: minTextHeight + 'px' } : {}
                    "
                >
                    ...
                    <div class="text-white flex flex-col space-y-6">
                        <span
                            v-for="(line, i) in slides[current].text.split(
                                '\n',
                            )"
                            :key="i"
                            >{{ line }}</span
            ...
            >
                <img
                    src="/icons/pil.svg"
                    :alt="$t('arrangementer.next')"
                    class="sm:w-10 sm:h-10 w-6 h-6 cursor-pointer transition"
                />
            </button>
        </div>
    </div>
