<template>
<div class="bg-black pb-2">
    <footer class="px-4 py-4 w-full max-w-4xl mx-auto text-center text-white text-sm opacity-70">
        © {{ new Date().getFullYear() }} Lasergame Center Oksbøl | CVR {{ settings.cvr }} | {{ settings.phone }} | Mønstervej 60, 6854 Henne
    </footer>
    <div class="flex flex-wrap justify-center gap-4 mb-4 px-4">
        <a href="https://www.facebook.com/lasergamecenter/" target="_blank" rel="noopener" class="text-white hover:text-neonred transition-colors duration-300">Facebook</a>
        <a v-if="settings.email" :href="`mailto:${settings.email}`" class="text-white hover:text-neonred transition-colors duration-300 break-all min-w-0">{{ settings.email }}</a>
        <NuxtLink to="/privatlivspolitik" class="text-white hover:text-neonred transition-colors duration-300">{{ $t('privatlivspolitik.title') }}</NuxtLink>
    </div>
</div>
</template>

<script setup>
const { settings, fetchSettings } = useSettings()
onMounted(fetchSettings)
</script>